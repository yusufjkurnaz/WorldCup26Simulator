...
// YENİ: Dil Desteği ve Ülke Çeviri Sözlüğü Bağlantısı
...
export function slotlariArayuzeCiz(slotlarData) {
    [1, 2, 3].forEach(id => {
        ...
        if (data === null) {
            ...
            // YENİ: Slot menüsündeki takım ismi de çevriliyor
...
export function HTMLGrupFiksturuOlustur(grupFiksturu, oynananMaclarHafizasi, globalMacGunu, kullaniciTakimi = null) {
    let html = '';
    [1, 2, 3].forEach(gun => {
        let gunMaclari = grupFiksturu.filter(m => m.macGunu === gun);
